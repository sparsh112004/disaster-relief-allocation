...
class DisasterReliefSystem:
    ...
    def update_warehouse_units(self):
        ...
        # Get warehouse ID with validation
        max_attempts = 3
        attempts = 0
        warehouse = None
        
        while attempts < max_attempts:
            wh_id = input("\nEnter Warehouse ID (e.g., WH1) or 'cancel' to exit: ").strip().upper()
            
            if wh_id.lower() == 'cancel':
                print("Operation cancelled.")
                input("\nPress Enter to continue...")
                return
                
            if not wh_id.startswith('WH') or not wh_id[2:].isdigit():
                print("❌ Invalid warehouse ID format. Should be 'WH' followed by numbers.")
                attempts += 1
                continue
            ...
            print(f"❌ Warehouse {wh_id} not found.")
            attempts += 1
            
        if not warehouse:
            print("❌ Maximum attempts reached. Operation cancelled.")
            input("\nPress Enter to continue...")
            return
        
        # Show current inventory of selected warehouse
        print(f"\nCurrent inventory for {warehouse.id}:")
        for item, quantity in warehouse.inventory.items():
            print(f"- {item.capitalize()}: {quantity} units")
        
        # Get resource to update with validation
        attempts = 0
        while attempts < max_attempts:
            resource = input("\nEnter resource name to update or 'cancel' to exit: ").lower().strip()
            ...
            print("Available resources:", ", ".join(warehouse.inventory.keys()))
            attempts += 1
        ...
        while attempts < max_attempts:
            try:
                quantity_input = input(f"Enter new quantity for {resource} (or 'cancel' to exit): ")
        ...
        confirm = input(f"\nConfirm update of {resource} from {old_quantity} to {new_quantity} units? (yes/no): ").lower()
        ...
        print(f"\n✅ Successfully updated {resource} in {warehouse.id}")
